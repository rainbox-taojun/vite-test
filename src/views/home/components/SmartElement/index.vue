...
const ballMove = () => {
  let main = document.getElementById('box');  //获取运动边界和10个div
  let circles = main.querySelectorAll('.item');
  let container = [];//存放10个球的每个具体信息，包括坐标，速度等值
  let arr = [];
  let maxW = 0;//初始化运动的最大宽和高，初始定义0
  let maxH = 0;
  let cwidth = circles[0].offsetWidth; //小球的直径

  //根据浏览器窗口的大小自动调节小球的运动空间
  window.onresize = function () {
    console.log(main.offsetWidth)
    maxW = main.offsetWidth - circles[0].clientWidth;//为了让小球不卡在浏览器边缘，
    maxH = main.offsetHeight - 38 - circles[0].clientHeight;// 所以要减去自身的宽高
  ...
  for (let i = 0; i < circles.length; i++) {
    ...
    arr.speed = 0 + Math.floor(Math.random() * 4);//随机速度
    arr.timer = null;//计时器
    arr.index = i;//索引值
